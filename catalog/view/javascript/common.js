...
$(document).ready(function() {
	...
	$('#home-search input[name=\'search\']').parent().find('button').on('click', function() {
		...
		var value = $('#home-search input[name=\'search\']').val();
	...
	$('#home-search input[name=\'search\']').on('keydown', function(e) {
		if (e.keyCode == 13) {
			$('#home-search input[name=\'search\']').parent().find('button').trigger('click');
		}
	});
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['success']) {
					...
					$('#cart > .cart-menu-container').load('index.php?route=common/cart/info');
				}
			},
	...
	'remove': function(key) {
		$.ajax({
			...
			success: function(json) {
				// Need to set timeout otherwise it wont update the total
				/*setTimeout(function () {
					$('button.cart-btn').html('<span id="cart-total"><i class="fa fa-shopping-cart"></i> ' + json['total'] + '</span>');
				}, 100);*/
